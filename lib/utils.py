...
def q_err(m, nbits=4, sz=32, scale=None, act_scale=None, t=False, H=None, o_shrink=True, ste=False, hamiltonian=None):
    ...
    if scale is not None:
        ...
        delta2 = delta
    if False: # hamiltonian is not None and t is False:
        h = block_diagonalize(hamiltonian).to(delta.dtype)
        ...
        if act_scale is not None:
            act_scale = act_scale.to(delta.dtype).to(delta.device)
        ...
        if t is True:
            ...
            delta2 = delta2.T
        if H is not None:
            delta = (delta.reshape(-1, H.shape[0]) @ H.T.to(delta.dtype)).reshape(delta.shape)
