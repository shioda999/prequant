...
def quantization_loss_for_smooth(As, Bs, chunk_size, H, s, ignore_act_scale=False):
    ...
    if hasattr(As[0], "act_scale") and ignore_act_scale is False:
        ...
        for i, B in enumerate(Bs):
            ...
            sb = B.weight.abs().pow(2).mean(dim=1).pow(0.5)
            losses.append(q_err(B.weight / s, scale=s * sa, act_scale=As[0].act_scale.sqrt(), o_shrink=False, H=H, hamiltonian=hamiltonian).reshape(-1, B.weight.shape[-1]).div(sb).sum(dim=0))
