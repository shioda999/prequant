...
def quantization_loss_for_smooth(As, Bs, chunk_size, H, s, ignore_act_scale=False):
    losses = []
    if False: #hasattr(As[0], "act_scale") and ignore_act_scale is False:
        ...
        for i, B in enumerate(Bs):
            ...
            # losses.append(q_err(B.weight / s, scale=s * sa, act_scale=As[0].act_scale, o_shrink=False, H=H, hamiltonian=hamiltonian).reshape(-1, B.weight.shape[-1]).sum(dim=0))
